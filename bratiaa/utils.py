...
class TokenOverlap:
    ...
    def overlapping_tokens(self, start, end):
        ...
        if start_token == -1:
            start_token = 0
        if self.tokens[start_token][1] <= start:
            start_token += 1
        end_token = self.char2token[end]
        if end_token < 0:
            return []
        if self.tokens[end_token][0] >= end:
            end_token -= 1
